          ...
          StreamBuilder(
            ...
            builder: (context, AsyncSnapshot<DatabaseEvent> snapshot) {
              ...
              if (snapshot.hasData && snapshot.data!.snapshot.value != null) {
                ...
                double turbidity = (data['turbidity'] ?? 1.0).toDouble();

                bool isTempSafe = temp >= 25.0 && temp <= 30.0;
                bool isPhSafe = ph >= 6.5 && ph <= 8.5;
                bool isTurbiditySafe = turbidity < 5.0;
                bool isGoRecommendation = isTempSafe && isPhSafe && isTurbiditySafe;
                    ...
                    GridView.count(
                      ...
                      children: [
                        _buildSensorCard(
                          ...
                          status: isTempSafe ? 'SAFE' : 'DANGER',
                          statusColor: isTempSafe ? const Color(0xFF2E7D32) : const Color(0xFFC62828),
                          bgColor: isTempSafe ? const Color(0xFFE8F5E9) : const Color(0xFFFFEBEE),
                        ),
                        _buildSensorCard(
                          title: 'Humidity',
                        ...
                        _buildSensorCard(
                          ...
                          status: isPhSafe ? 'SAFE' : 'CAUTION',
                          statusColor: isPhSafe ? const Color(0xFF2E7D32) : const Color(0xFFE65100),
                          bgColor: isPhSafe ? const Color(0xFFE8F5E9) : const Color(0xFFFFF3E0),
                        ),
                        _buildSensorCard(
                          title: 'Turbidity',
                          ...
                          status: isTurbiditySafe ? 'SAFE' : 'CAUTION',
                          statusColor: isTurbiditySafe ? const Color(0xFF2E7D32) : const Color(0xFFE65100),
                          bgColor: isTurbiditySafe ? const Color(0xFFE8F5E9) : const Color(0xFFFFF3E0),
                        ),
                      ],
                    ),
                  ],
                ...
                return const Center(
                  child: Padding(
                    padding: EdgeInsets.all(32.0),
                    child: Text("Waiting for hardware connection..."),
                  ),
                );
              }
            },
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(isGoRecommendation ? 'GO' : 'NO-GO', style: TextStyle(fontSize: 64, fontWeight: FontWeight.w900, color: isGoRecommendation ? const Color(0xFF2E7D32) : const Color(0xFFC62828), height: 1.1)),
          const SizedBox(height: 8),
          Text(isGoRecommendation ? 'Water parameters are within safe thresholds.' : 'Unsafe conditions detected. Do not release.', textAlign: TextAlign.center, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: isGoRecommendation ? Colors.green.shade800 : Colors.red.shade800)),
        ],
      ),
    );
  }
